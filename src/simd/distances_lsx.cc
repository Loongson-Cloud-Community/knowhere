...
fvec_inner_products_ny_lsx(float* ip, const float* x, const float* y, size_t d, size_t ny) {
    for (size_t i = 0; i < ny; i++) {
        ip[i] = fvec_inner_product_lsx(x, y, d);
        y += d;
    }
}
...
void
fvec_L2sqr_ny_transposed_lsx(float* __restrict dis, const float* __restrict x, const float* __restrict y,
...
/// return 0 if ny == 0.
size_t
fvec_L2sqr_ny_nearest_lsx(float* __restrict distances_tmp_buffer, const float* __restrict x, const float* __restrict y,
                          size_t d, size_t ny) {
...
/// return 0 if ny == 0.
size_t
fvec_L2sqr_ny_nearest_y_transposed_lsx(float* __restrict distances_tmp_buffer, const float* __restrict x,
...
void
fvec_inner_product_batch_4_lsx(const float* __restrict x, const float* __restrict y0, const float* __restrict y1,
...
void
fvec_L2sqr_batch_4_lsx(const float* x, const float* y0, const float* y1, const float* y2, const float* y3,
...
float
fvec_inner_product_bf16_patch_lsx(const float* x, const float* y, size_t d) {
    float res = 0;
    for (size_t i = 0; i < d; i++) {
...
float
fvec_L2sqr_bf16_patch_lsx(const float* x, const float* y, size_t d) {
    float res = 0;
    for (size_t i = 0; i < d; i++) {
...
void
fvec_inner_product_batch_4_bf16_patch_lsx(const float* __restrict x, const float* __restrict y0,
...
void
fvec_L2sqr_batch_4_bf16_patch_lsx(const float* x, const float* y0, const float* y1, const float* y2, const float* y3,
                                  const size_t d, float& dis0, float& dis1, float& dis2, float& dis3) {
    float d0 = 0, d1 = 0, d2 = 0, d3 = 0;
